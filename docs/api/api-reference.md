...
> 维护日期：2026-08-26 · 覆盖 `apps/web/app/api/` 全部 **30 个 route 文件**
...
## 三、发布 / 反馈 / Skills / Wiki / Settings / 其他（19 route，含 3 个 LLM 端点详见第四节）
...
| POST | `/api/agents/[id]/chat` | `{ message: 1-4000 字, history?: [{role: user\|assistant, content}]（≤20 条） }` | `{ reply, model, usage, latencyMs }`；会话只存前端内存 |

试聊会以该 Agent **当前** Prompt 分区配置（systemPrompt + 角色定义 + 约束 + 输出格式）作为 system prompt——改完配置立即可验。

## 五、测试口径

全部 API 有 Vitest 集成测试（`app/api/__tests__/`，7 个文件）：验证 status / body / 审计副作用；
