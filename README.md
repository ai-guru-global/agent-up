...
│       │   ├── api/                # API 路由（34 个 route 文件，含 5 个真实 LLM 端点）
...
```mermaid
flowchart LR
    P["浏览器 · App Router 页面<br/>（见页面导航）"] --> A["API 路由层 · 34 个 route<br/>Zod 校验 · 结构化错误 · Actor 上下文"]
...
| `docs/api/api-reference.md` | API 接口完整说明（34 route + 错误码 + LLM 端点细节） |
