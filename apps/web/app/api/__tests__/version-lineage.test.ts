...
function withMetadata(prompts: Record<string, unknown> | null) {
  return prompts
...
async function writeVersion(
  ...
  await seedReleaseWithVersion({
    ...
    changeNote,
    snapshots: { prompt: withMetadata(prompts) },
  });
}
...
describe("GET /api/agents/[id]/version-lineage（资产演进历史）", () => {
  ...
  it("单版本 = 基线：changedPartitions 与 diffSummary 为 null", async () => {
    await seedAgent("agent-one", "基线助手");
    await writeVersion("ver-a", "agent-one", "0.1.0", "2026-09-01T09:00:00.000Z", {
      systemPrompt: "v1",
    });
    ...
    expect(json.data.agentName).toBe("基线助手");
  ...
  it("相邻版本 diff：PROMPT 增/删/改计数正确；未变分区不进 changedPartitions；元数据键不参与比较", async () => {
    await seedAgent("agent-lineage", "演进助手");
    await writeVersion("ver-v1", "agent-lineage", "0.1.0", "2026-09-01T09:00:00.000Z", {
      systemPrompt: "你是 ECS 助手",
      constraints: ["不回答无关问题"],
    });
    await writeVersion("ver-v2", "agent-lineage", "0.2.0", "2026-09-02T09:00:00.000Z", {
  ...
  it("多版本乱序写入仍按 publishedAt 升序组成演进线；跨 Agent 版本不串", async () => {
    await seedAgent("agent-sorted", "排序助手");
    await seedAgent("agent-other", "别家助手");
    await writeVersion("ver-s2", "agent-sorted", "0.2.0", "2026-09-03T09:00:00.000Z", { systemPrompt: "v2" });
    await writeVersion("ver-o1", "agent-other", "9.9.9", "2026-09-02T09:00:00.000Z", { systemPrompt: "别家" });
    await writeVersion("ver-s1", "agent-sorted", "0.1.0", "2026-09-01T09:00:00.000Z", { systemPrompt: "v1" });
    await writeVersion("ver-s3", "agent-sorted", "0.3.0", "2026-09-04T09:00:00.000Z", { systemPrompt: "v2" }); // 与 v2 相同 → 无变更
  ...
  it("快照缺失（null）→ 视为空对象参与 diff（内容出现记 added）", async () => {
    await seedAgent("agent-null", "空快照助手");
    await writeVersion("ver-n1", "agent-null", "0.1.0", "2026-09-01T09:00:00.000Z", null);
    await writeVersion("ver-n2", "agent-null", "0.2.0", "2026-09-02T09:00:00.000Z", { systemPrompt: "首次配置" });
