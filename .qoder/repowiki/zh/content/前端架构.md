...
## 更新摘要
**变更内容**
- 新增 /maas 页面作为仪表板布局的一部分，提供 MaaS（模型即服务）集成的可视化界面
- 在侧边栏导航中添加了"模型服务"菜单项，指向新的 MaaS 页面
- 实现了完整的 MaaS 产品矩阵展示、模型用量统计、路由策略可视化和四分区集成点说明
- 集成了 Mermaid 图表组件用于展示模型路由流程图
- 增强了仪表板布局的导航系统，支持更多功能模块
...
**最新更新**：新增了 MaaS（模型即服务）集成页面，作为仪表板布局的重要组成部分，提供阿里云大模型产品的可视化集成界面，包括产品矩阵、模型用量监控、路由策略配置和四分区集成点说明。
...
A --> L["(dashboard)/maas/page.tsx"]
A --> M["api/dashboard/route.ts"]
A --> N["api/agents/route.ts"]
A --> O["api/feedback/route.ts"]
A --> P["api/releases/route.ts"]
A --> Q["api/auth/login/route.ts"]
A --> R["layout.tsx"]
A --> S["globals.css"]
T["apps/web/lib/utils.ts"] --> M
T --> N
T --> O
T --> P
T --> Q
U["apps/web/postcss.config.mjs"] --> S
V["apps/web/next.config.ts"] --> A
W["apps/web/eslint.config.mjs"] --> T
X["apps/web/tsconfig.json"] --> T
```

**图表来源**
- [apps/web/app/layout.tsx:1-20](file://apps/web/app/layout.tsx#L1-L20)
- [apps/web/app/(dashboard)/layout.tsx:1-104](file://apps/web/app/(dashboard)/layout.tsx#L1-L104)
...
- [apps/web/app/(dashboard)/maas/page.tsx:1-106](file://apps/web/app/(dashboard)/maas/page.tsx#L1-L106)
...
**更新**：新增了认证流程和 MOCK 标识系统，增强了用户体验和数据源透明度。**新增**了 MaaS 集成页面，完善了企业级 AI 应用的完整解决方案。
...
- [apps/web/app/(dashboard)/architecture/_components/ui.tsx:1-229](file://apps/web/app/(dashboard)/architecture/_components/ui.tsx#L1-L229)
- [apps/web/app/api/auth/login/route.ts:1-24](file://apps/web/app/api/auth/login/route.ts#L1-L24)
- [apps/web/app/api/dashboard/route.ts:1-40](file://apps/web/app/api/dashboard/route.ts#L1-L40)
- [apps/web/app/api/agents/route.ts:1-46](file://apps/web/app/api/agents/route.ts#L1-L46)
- [apps/web/app/api/feedback/route.ts:1-70](file://apps/web/app/api/feedback/route.ts#L1-L70)
