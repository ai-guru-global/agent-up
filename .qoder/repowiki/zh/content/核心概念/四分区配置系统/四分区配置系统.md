...
- [route.ts](file://apps/web/app/api/agents/[id]/config/[partition]/route.ts)
- [page.tsx](file://apps/web/app/(dashboard)/architecture/page.tsx)
- [agent-service.ts](file://apps/web/lib/services/agent-service.ts)
</cite>

## 更新摘要
**变更内容**   
- 新增四分区配置系统的可视化表示章节，展示 Prompt、Knowledge、Tools、Routing 四个分区的配置模型与归因分类学双重价值
- 强化失败工单归因分析机制，说明四分区如何作为调试和故障排查的分类框架
- 更新架构图表，增加可视化展示四分区配置系统的整体设计
- 补充归因分类学的实际应用案例和使用场景
...
**重要更新**：四分区配置系统不仅是配置模型，更是失败的归因分类学。在复盘一张失败工单时，归因结论最终都会落到某个分区：知识缺失 → Knowledge，工具失败 → Tools，回答越界 → Prompt，分派错误 → Routing。这种双重价值是它比"把配置散落在不同页面"更深的意义。

## 项目结构
Agent Up 采用前后端同仓（Monorepo）组织方式：
- 前端 Next.js 应用位于 apps/web，包含页面路由与 API 路由。
- 共享类型定义位于 packages/shared，统一对外暴露 Agent 相关类型与常量。
- 数据库模型与迁移由 Prisma 管理，位于 packages/db/prisma/schema.prisma。

```mermaid
graph TB
subgraph "前端界面"
UI["配置中心页面<br/>四分区标签页"]
Visual["可视化表示<br/>归因分类学视图"]
end
subgraph "API层"
API["分区配置API<br/>/api/agents/:id/config/:partition"]
Validation["输入验证<br/>Zod Schema校验"]
end
subgraph "服务层"
Service["Agent服务<br/>配置读写操作"]
Audit["审计记录<br/>变更追踪"]
end
subgraph "数据持久化"
DB["Prisma Schema<br/>四分区配置模型"]
Store["JSON存储<br/>运行时数据"]
end
UI --> API
Visual --> UI
API --> Validation
...
**图表来源**
- [page.tsx:134-172](file://apps/web/app/(dashboard)/architecture/page.tsx#L134-L172)
- [route.ts:1-85](file://apps/web/app/api/agents/[id]/config/[partition]/route.ts#L1-L85)
- [schemas.ts:23-60](file://apps/web/lib/schemas.ts#L23-L60)
- [agent-service.ts:129-169](file://apps/web/lib/services/agent-service.ts#L129-L169)
...
- **校验**：基于共享类型与 Zod Schema 对输入进行强类型与规则校验。
...
**图表来源**
- [page.tsx:134-172](file://apps/web/app/(dashboard)/architecture/page.tsx#L134-L172)
- [route.ts:46-85](file://apps/web/app/api/agents/[id]/config/[partition]/route.ts#L46-L85)
- [agent-service.ts:137-169](file://apps/web/lib/services/agent-service.ts#L137-L169)
- [agent-service.ts:176-208](file://apps/web/lib/services/agent-service.ts#L176-L208)
...
**图表来源**
- [schema.prisma:103-114](file://packages/db/prisma/schema.prisma#L103-L114)
...
**图表来源**
- [schema.prisma:116-145](file://packages/db/prisma/schema.prisma#L116-L145)
...
**图表来源**
- [schema.prisma:161-173](file://packages/db/prisma/schema.prisma#L161-L173)
...
**图表来源**
- [schema.prisma:175-214](file://packages/db/prisma/schema.prisma#L175-L214)
- [schema.prisma:298-354](file://packages/db/prisma/schema.prisma#L298-L354)
- [release.ts:1-36](file://packages/shared/src/types/release.ts#L1-L36)
...
### 四分区配置模型的可视化表示
四分区配置系统在前端界面提供了直观的可视化表示，帮助用户理解每个分区的职责和作用：
...
### 归因分类学的应用
四分区配置系统不仅是配置模型，更是失败的归因分类学。在复盘一张失败工单时，归因结论最终都会落到某个分区：

| 分区 | 类比 | 典型失败场景 | 归因方向 |
|------|------|-------------|----------|
| **Prompt** | Agent 的「性格与纪律」 | 回答风格不对、越界承诺、格式混乱 | 检查系统提示词、角色定义、约束条件 |
| **Knowledge** | Agent 的「长期记忆」 | 知识缺失、知识过期、检索不中 | 检查知识库关联、检索策略、置信度阈值 |
| **Tools** | Agent 的「手」 | 工具调用失败、缺少某类能力 | 检查 MCP 工具配置、Wiki 查询工具、并发设置 |
| **Routing** | Agent 的「分诊台」 | 不该接的接了、该转人工没转 | 检查路由规则、默认动作、人工阈值 |

### 可视化归因分析流程
```mermaid
flowchart TD
Start(["失败工单分析"]) --> Collect["收集失败案例"]
Collect --> Classify{"归因分类"}
Classify --> |知识相关问题| Knowledge["Knowledge 分区"]
Classify --> |工具调用失败| Tools["Tools 分区"]
Classify --> |回答质量问题| Prompt["Prompt 分区"]
Classify --> |分派错误| Routing["Routing 分区"]
Knowledge --> AnalyzeK["分析知识配置"]
Tools --> AnalyzeT["分析工具配置"]
Prompt --> AnalyzeP["分析提示词配置"]
Routing --> AnalyzeR["分析路由配置"]
AnalyzeK --> FixK["修复知识配置"]
AnalyzeT --> FixT["修复工具配置"]
AnalyzeP --> FixP["修复提示词配置"]
AnalyzeR --> FixR["修复路由配置"]
FixK --> Verify["验证修复效果"]
FixT --> Verify
FixP --> Verify
FixR --> Verify
...
**图表来源**
- [schema.prisma:61-91](file://packages/db/prisma/schema.prisma#L61-L91)
- [schema.prisma:103-173](file://packages/db/prisma/schema.prisma#L103-L173)
- [route.ts:14-26](file://apps/web/app/api/agents/[id]/config/[partition]/route.ts#L14-L26)
...
- [schema.prisma:332-354](file://packages/db/prisma/schema.prisma#L332-L354)

## 结论
四分区配置系统通过清晰的职责边界与统一的数据契约，实现了 Agent 行为的模块化与可演进。借助草稿、审计、版本与发布审批机制，平台在保证灵活性的同时提升了可控性与可追溯性。**更重要的是，四分区配置系统不仅是配置模型，更是归因分类学**——在复盘失败工单时，能够明确地将问题定位到具体的配置分区，从而指导针对性的修复和优化。遵循本文的最佳实践与性能建议，开发者可以高效构建稳定可靠的 Agent 能力。
...
- **归因分析**：利用四分区分类学进行失败案例分析，持续优化各分区配置。