...
- **新增AI智能发布审批辅助功能**：通过LLM分析配置差异，自动生成变更摘要和风险评估
...
- 服务层：release-service.ts 实现了核心的发布审批逻辑，effectiveness-service.ts 处理效果报告，llm-service.ts 提供AI能力
...
UI_Release["发布审批页<br/>AI摘要+Diff视图"]
end
subgraph "后端API"
...
- [apps/web/app/(dashboard)/releases/page.tsx:1-396](file://apps/web/app/(dashboard)/releases/page.tsx#L1-L396)
...
- [apps/web/lib/services/llm-service.ts:1-134](file://apps/web/lib/services/llm-service.ts#L1-L134)

## 核心组件
- **增强的发布审批（Release）**：记录一次发布申请的状态流转（待审批、已通过、已驳回、需修改），包含配置快照和最终生成的版本快照
- **AI智能发布审批辅助**：通过LLM分析配置差异，自动生成变更摘要和风险评估，辅助审批决策
...
## 架构总览
版本控制的核心流程围绕"草稿 → 提交发布 → AI分析 → 审批 → 生成版本快照 → 可回滚"展开。增强的发布审批工作流支持四种状态，AI智能辅助功能通过分析配置差异自动生成变更摘要和风险评估，不可变配置快照确保版本一致性，效果报告服务提供版本上线后的质量评估，双轨回滚机制保障快速恢复能力。

```mermaid
sequenceDiagram
participant Dev as "开发者/运维"
participant UI as "Agent 详情页"
participant ReleaseUI as "发布审批页"
...
Dev->>ReleaseUI : 在"发布审批"页查看待审批项
ReleaseUI->>API : GET /api/releases/[id] (获取发布详情)
API->>DB : 读取Release和baseline
ReleaseUI->>API : POST /api/releases/[id]/summary (AI分析)
API->>LLM : chatCompletion(配置差异分析)
LLM->>API : 返回变更摘要和风险评估
API->>ReleaseUI : 返回AI分析结果
Note over Dev,ReleaseUI : 查看AI辅助决策信息
Dev->>ReleaseUI : 进行审批决策
ReleaseUI->>API : PUT /api/releases/[id]/review (审批)
...
### AI智能发布审批辅助
新增的AI智能发布审批辅助功能为审批决策提供了智能化的支持：

#### 功能特性
- **智能变更分析**：自动分析四分区配置差异（Prompt/Knowledge/Tools/Routing）
- **变更摘要生成**：用2-3句话概括本次改了什么、意图是什么
- **风险评估**：列出1-3条审批时需要关注的风险或验证建议
- **无状态设计**：每次调用实时生成，不持久化存储结果
- **多模型支持**：基于小米MiMo LLM服务，支持自定义模型配置

#### 工作流程
1. **数据准备**：读取Release的configSnapshot和baseline（前一版本）
2. **差异提取**：对比四分区配置，提取变更内容
3. **AI分析**：调用LLM服务进行智能分析
4. **结果展示**：在前端展示AI生成的摘要和风险提示

```mermaid
flowchart TD
Start["点击AI变更摘要"] --> FetchData["获取Release和baseline数据"]
FetchData --> ExtractChanges["提取四分区配置差异"]
ExtractChanges --> PreparePrompt["构建AI分析提示词"]
PreparePrompt --> CallLLM["调用LLM服务"]
CallLLM --> Analyze["AI分析变更内容和风险"]
Analyze --> ReturnResult["返回变更摘要和风险评估"]
ReturnResult --> Display["前端展示AI分析结果"]
Display --> Decision["辅助审批决策"]
```

**图表来源**
- [apps/web/app/api/releases/[id]/summary/route.ts:28-79](file://apps/web/app/api/releases/[id]/summary/route.ts#L28-L79)
- [apps/web/lib/services/llm-service.ts:67-134](file://apps/web/lib/services/llm-service.ts#L67-L134)

**章节来源**
- [apps/web/app/api/releases/[id]/summary/route.ts:1-91](file://apps/web/app/api/releases/[id]/summary/route.ts#L1-L91)
- [apps/web/lib/services/llm-service.ts:1-134](file://apps/web/lib/services/llm-service.ts#L1-L134)
- [apps/web/app/(dashboard)/releases/page.tsx:188-257](file://apps/web/app/(dashboard)/releases/page.tsx#L188-L257)
...
3. **AI分析**：可选的AI智能分析，生成变更摘要和风险评估
4. **审批处理**：管理员在"发布审批"页面进行通过/拒绝/要求修改
5. **自动版本创建**：审批通过后，系统从 configSnapshot 自动生成 AgentVersion
6. **版本发布**：记录 publishedAt/publishedBy/changeNote，更新 Release 状态为 APPROVED
...
- 支持原始JSON和摘要视图切换
- **新增AI摘要视图**：通过AI分析生成变更摘要，提供更直观的变更理解
...
- **前端依赖**：
  ...
  - 发布审批页提供"待审批/已通过/已驳回/需修改"过滤和AI摘要功能
  - 版本历史组件支持效果标签展示和一键回滚
- **后端依赖**：
...
- **AI调用优化**：AI摘要功能为实时调用，应考虑缓存策略和超时处理
- **LLM限流**：注意LLM服务的调用频率限制和错误重试机制
...
- **AI摘要失败**：检查LLM服务配置（MIMO_API_KEY）、网络连接、API配额限制
- **LLM调用超时**：检查网络连通性、API响应时间、超时配置设置
...
## 结论
本版本控制系统以"草稿—提交—AI分析—审批—快照—回滚—审计"为主线，结合增强的发布审批工作流、AI智能发布审批辅助、不可变配置快照机制、效果报告服务和双轨回滚机制，提供了完整的 Agent 配置生命周期管理能力。通过清晰的 API 接口、严谨的数据模型与完善的审计日志，既满足运维人员的稳定性需求，也为开发者提供了最佳实践与技术细节参考。新增的AI智能发布审批辅助功能为版本质量评估提供了智能化支撑，显著提升了审批效率和准确性。

## 附录
- **术语表**
  - 分区（Partition）：PROMPT/KNOWLEDGE/TOOLS/ROUTING
  - 发布审批（Release）：发布申请的审批单，支持四种状态
  - AI变更摘要：通过LLM分析的配置差异，自动生成变更说明和风险评估
  ...
  - LLM服务：小米MiMo AI服务，提供智能分析和文本生成能力